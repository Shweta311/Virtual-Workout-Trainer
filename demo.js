...
function changesckelton(){
  if(showskelton == true)
    showskelton =false;
  else showskelton =true;
}
...
function gotResult(error, results) {
  ...
  if(num_sets < max_num_sets && tab != 'endofset'){
    ...
    if (results[0].confidence > 0.75) {
      ...
      if(poseLabel == '2' && repsflag ){
        ...
        workouts[Workout_num][num_sets + 4] = num_reps;
        repsflag = false;
  ...
  if (pose) {
   ...
   // let Trainer = [174.64312311809982, 69.53778684789786, 89.77495008335046, 104.05599353911487, 92.18559648338066, 99.85785508345825, 76.14903878571451, 79.17080908761324, 95.73438592016154, 100.60119004353714, 92.98789493924744, 0];
    ...
    let cur_label = 0;
     ...
     console.log("here is pposlabel :",cur_label," ", poseLabel);
      console.log("here is trainer counter ",trainercounter);
...
function giveSummary(){
  ...
  tab = 'endofset'
  for(var i =0;i<workouts[Workout_num][2];i++){

    addDataToElements("summary-output"  , workouts[Workout_num][0] , i+1,workouts[Workout_num][i + 4],workouts[Workout_num][3]);
...
function addDataToElements(idname , workoutName , setnumber ,totalreps ,totalassignedreps){
  ...
  l1u2.textContent = "Total Completed reps :" + totalreps;
  l1u.appendChild(l1u2);

  const l1u3 = document.createElement("li");
  l1u3.textContent = " Total Assigned Reps :" + totalassignedreps;
